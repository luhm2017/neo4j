...
//  }
  ...
  def createNodeSetProperty(graphdb: GraphDatabaseService, list: java.util.List[String]): Unit = {
    ...
    val msgphone = list.get(17)  //短信接收手机号码
    val telecode = list.get(18)
    val deviceId = list.get(19)  //手机IMEI
    //添加订单属性
    val currentDueDay = list.get(20) //当前逾期天数
    val historyDueDay  = list.get(21) //历史最大逾期天数
    val performance = list.get(22) //申请状态名称 ，Q标拒绝或者other_refuse
    val rufuseType = list.get(23)  //拒绝状态
    val productNo = list.get(24)  //产品编码
    val  productName= list.get(25) // 产品名称
    val  applyState = list.get(26) //申请结果状态
    val applyLastState = list.get(27) //申请结果
    val failReason = list.get(28)  //失败原因
    ...
    //创建设备号，过滤设备号异常数据
    if (StringUtils.isNotEmpty(deviceId)&& !"\\N".equals(deviceId)&& !"-".equals(deviceId)) {
    ...
    //申请手机号
    if (StringUtils.isNotEmpty(mobile)&& !"\\N".equals(mobile)&& !"-".equals(mobile)) {
    ...
    //邮箱
    if (StringUtils.isNotEmpty(email)&& !"\\N".equals(email)&& !"-".equals(email)) {
    ...
    //贷款银行卡
    if (StringUtils.isNotEmpty(loanPan)&& !"\\N".equals(loanPan)&& !"-".equals(loanPan)&& !"ds!".equals(loanPan)) {
    ...
    //还款银行卡
    /*if (StringUtils.isNoneEmpty(returnPan)&& !"\\N".equals(returnPan)&& !"-".equals(returnPan)&& !"ds!".equals(returnPan)) {
      var returnBankCard: Option[Node] = nodeMap.get(returnPan)
      if (!returnBankCard.isDefined) {
        val tmpNode = graphdb.createNode(Labels.BankCard)
        tmpNode.setProperty(CommonConstant.CONTENTKEY, returnPan)
        nodeMap.put(returnPan, tmpNode)
        returnBankCard = nodeMap.get(returnPan)
      }
      apply.createRelationshipTo(returnBankCard.get, RelationshipTypes.RETURNBANKCARD)
    }*/

    //联系人电话
    if (StringUtils.isNotEmpty(contactMobile)&& !"\\N".equals(contactMobile)&& !"-".equals(contactMobile)) {
    ...
    //紧急联系人电话
    if (StringUtils.isNotEmpty(emergencyMobile)&& !"\\N".equals(emergencyMobile)&& !"-".equals(emergencyMobile)) {
    ...
    //单位电话 companyPhone
    if (StringUtils.isNotEmpty(companyPhone)&& !"\\N".equals(companyPhone)&& !"-".equals(companyPhone)) {
      //剔除单位电话中特殊字符
